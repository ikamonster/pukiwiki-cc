...
cc.inc.php, v1.00 2020 M.Taniguchi
License: CC BY 4.0

クリエイティブ・コモンズ・ライセンス（4.0）のコモンズ証へのリンクアイコンを表示するプラグイン。
...
function plugin_cc_make($args) {
	...
	list($license, $size) = $args;
	$license = strtolower($license);
	$body = '';

	if (isset($licenseTbl[$license])) {
		$size = ($size)? (float)$size : 100;
		$url = 'https://creativecommons.org/licenses/' . $license . '/4.0/' . ((LANG == 'ja')? 'deed.ja' : '');

		$body = '<a class="_p_cc" href="' . $url . '" title="' . $licenseTbl[$license] . '" rel="noopener nofollow external license" style="font-size:' . $size . '%">' . $svgTbl['cc'];
		foreach (explode('-', $license) as $key) $body .= $svgTbl[$key];
		$body .= '</a>';

		static	$included = false;
		if ($body && !$included) {
			$col = (PLUGIN_CC_COLOR)? 'fill:' . PLUGIN_CC_COLOR : 'fill:#000;fill:var(--color,#000)';
			$body .= '<style>._p_cc{display:inline-block;height:1.5em;width:auto;overflow:visible;vertical-align:middle}._p_cc svg{height:100%;width:auto;overflow:visible;margin-right:.25em;stroke:none;' . $col . '}._p_cc svg:last-child{margin-right:0}</style>';
			$included = true;
		}
	}

	return ($body)? $body : '&cc(by|by-sa|by-nd|by-nc|by-nc-sa|by-nc-nd[,scale]);';
}
